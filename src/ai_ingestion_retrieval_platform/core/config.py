...
class Settings(BaseSettings):
    ...
    max_redirects: int = Field(
        default=5,
        ge=0,
    )  # Max redirect hops per URL fetch.
    max_preview_bytes: int = Field(
        default=65_536,
        ge=1,
    )  # Max response bytes to read.
    max_preview_text_chars: int = Field(
        default=500,
        ge=1,
    )  # Max preview text returned.
    max_batch_urls: int = Field(
        default=20,
        ge=1,
    )  # Max URLs accepted per batch request.
    allowed_fetch_ports: tuple[int, ...] = (
        80,
        443,
    )  # Allowed outbound URL ports.
    ...
    retry_attempts: int = Field(
        default=3,
        ge=1,
    )  # Max retry attempts per URL fetch.
    ...
    # main.py, persistence/engine.py, api/routes/health.py
    database_enabled: bool = False
    # Enable PostgreSQL-backed persistent ingestion.

    database_url: str = "postgresql+asyncpg://localhost:5432/ai_irp"
    ...
    # Max persistent PostgreSQL connections per application process.

    database_pool_timeout_seconds: float = Field(
        default=5.0,
        gt=0,
    )
    # Max wait for an available pooled database connection.

    database_connect_timeout_seconds: float = Field(
        default=10.0,
        gt=0,
    )
    # Max time to establish a PostgreSQL connection.

    database_readiness_timeout_seconds: float = Field(
        default=2.0,
        gt=0,
    )
    # Max total time allowed for the PostgreSQL readiness probe.

    database_echo_sql: bool = False
    # Emit SQL through SQLAlchemy logging. Intended for local debugging only.
    ...
    def validate_setting_relationships(self) -> Self:
        ...
        if self.database_enabled:
            ...
            if not database_url:
                raise ValueError(
                    "database_url is required when database_enabled is true"
                )
